...
void move(const v8::FunctionCallbackInfo<v8::Value>& info)
{
	...
	work->src = generic_path(to_string<_char_t>(info[0].As<v8::String>()));
	...
	work->dst = generic_path(to_string<_char_t>(info[1].As<v8::String>()));
	...
	uv_queue_work(uv_default_loop(), &work->handle, move_async, move_complete);
}
