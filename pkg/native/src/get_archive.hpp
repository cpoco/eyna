...
static void get_archive_complete(uv_work_t* req, int status)
{
	...
	for (auto& [_, ent] : work->m) {
		...
		obj->Set(CONTEXT, to_string(V("ctime")), v8::BigInt::New(ISOLATE, ent.ctime));
		obj->Set(CONTEXT, to_string(V("mtime")), v8::BigInt::New(ISOLATE, ent.mtime));
...
void get_archive(const v8::FunctionCallbackInfo<v8::Value>& info)
{
	...
	work->max_depth = work->min_depth + info[2].As<v8::Int32>()->Value();
