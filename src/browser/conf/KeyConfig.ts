...
export namespace Command {
	...
	export type CodeBind = Record<number, WhenBind>
	export type WhenBind = Partial<Record<When, KeyBind>>
	export type KeyBind = {
		when: When
		cmd: string[]
		prm: string[]
...
class Key extends Abstract<FileFormat> {
	...
	set whenType(when: Command.When) {
		switch (when) {
			case Command.When.Always:
				break
			case Command.When.Filer: // fallthrough
			case Command.When.Modal: // fallthrough
	...
	postLoad() {
		...
		for (const k of this.data.key) {
			try {
				...
				Util.merge(this.codeBind, { [code]: wb })
			}
			catch (err) {
				console.error(err)
			}
	...
	get(input: electron.Input): Command.KeyBind | null {
		const code = inputToCode(input)
		return this.codeBind[code]?.[Command.When.Always] ?? this.codeBind[code]?.[this.when] ?? null
	}
}

export const KeyConfig = new Key()

function toArray(s: string | string[] | undefined): string[] {
	return Util.isString(s) ? [s] : Util.isArray(s) ? s : []
}
...
function acceleratorToCode(key: string | string[]): number {
	...
	for (const k of toArray(key).map((k) => k.toLowerCase())) {
		ret |= (FlagMap[k] ?? 0) << 12
		ret |= CodeMap[k] ?? 0
		ret |= KeyMap[k] ?? 0
	}
